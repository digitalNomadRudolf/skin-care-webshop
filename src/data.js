...
const cats = productItems.reduce((curCats, { category, title, img, info }) => {
    if (!curCats[category]) curCats[category] = [];
    curCats[category].push(category, img, title, info);
    return curCats;
}, []);
/* const productCats = Object.entries(cats); */
/* const productCats = Array.from(Object.keys(cats), k=>[`${k}`, cats[k]]); */
/* const productCats = Object.keys(cats); */

/* const productCats = _.groupBy(products, 'category'); */

const catGroup = function(data, key) {
